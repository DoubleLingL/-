...
def geturl(keyword):
    ...
    for i in range(1,varglobal.COUNTCELL+1):
        ...
        #l.append(newgrid1)
        print(str(r[0])+','+str(r[2])+','+str(r[1])+','+str(r[3]))
        url1 = "http://api.map.baidu.com/place/v2/search?query=" + keyword + "& bounds=" + str(r[0]) + ',' + str(r[2]) + ',' + str(r[1]) + ',' + str(r[3]) + "&page_size=20&output=json&ak=Mb4ucIMVc3sSxsmkzh4NM4t73EUDFrjv"
        html = requests.get(url1)  # 获取网页信息
        data = html.json()  # 获取网页信息的json格式数据
        totalPOI = data['total']
        #一页最多爬取20个POI，最多20页也即是最多400个POI ，page_nums为20的倍数
        pagenums=int((totalPOI+varglobal.page_nums-1)/varglobal.page_nums)
        print('page_nums:',pagenums)
        for i in range(0,pagenums):
            page_num=str(i)
            url="http://api.map.baidu.com/place/v2/search?query="+keyword+"& bounds="+str(r[0])+','+str(r[2])+','+str(r[1])+','+str(r[3])+"&page_size=20&page_num="+str(page_num)+"&output=json&ak=Mb4ucIMVc3sSxsmkzh4NM4t73EUDFrjv"
            #print('url',url)
            urls.append(url)
    print('urls', urls)
    return urls
...
def getPOI(urls):
    ...
    for url in urls:
        ...
        path = '/home/hll/PycharmProjects/POI/data/CenterCityPOIInfo_drom.csv'
...
if __name__=="__main__":
    #keywords=['宿舍','公司企业','学校','美食','购物','休闲娱乐','景区','其他','政府机构']
    #keywords = ['学校','政府机构','医疗']
    keywords = ['宿舍']
    allinfo=[]
    ...
    print(info)
    #info.to_csv('/home/hll/PycharmProjects/POI/data/info1.csv',header=True,
                #encoding='utf_8',index=False)
    '''
    #df = pd.DataFrame(columns=["LivePlace", "DutyPlace", "Diet", "Shopping", "Amuse"])
    df = pd.DataFrame(columns=["LivePlace"])
    df['LivePlace'] = info['宿舍']
    #df['DutyPlace'] = info.apply(lambda x: x['公司企业'] + x['学校'], axis=1)
    #df['Diet'] = info['美食']
    #df['Shopping'] = info['购物']
    ...
    #df['Other'] = info['其他']
    print(df)
    df.to_csv('/home/hll/PycharmProjects/POI/data/statistic_drom.csv',header=True,
                encoding='utf_8',index=False)
'''