...
numpy.random.seed(7)
# load the dataset
dataframe = read_csv('/media/hll/amuse/gy/predict23142/flow24/flow.csv',header=0)
...
#adam=Adam(lr=0.05,beta_1=0.9, beta_2=0.999,decay=0.01, epsilon=1e-08)

#sgd = SGD(lr=learning_rate, momentum=momentum, decay=decay_rate, nesterov=False)
#model.compile(loss='binary_crossentropy', optimizer=sgd, metrics=['accuracy'])
'''
####随着迭代次数增加，训练很慢，采用学习速率衰减运行时间也差不多，RMSE结果差别不大，但要好一点######
'''
...
def presicion(accu,b,c):
    ...
    print(format(float(presicion)/float(379),'.6f'))
presicion(0.1,b,c)
presicion(0.05,b,c)
presicion(0.025,b,c)
presicion(0.02,b,c)
...
save.to_csv('/media/hll/amuse/gy/predict23142/flow24/inflowpre.csv',index=False,sep=',')
